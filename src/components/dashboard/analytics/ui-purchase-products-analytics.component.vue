<template>
    ...
    <div :class="baseClass">
        <div :class="`${baseClass}__header`">
            <h1 :class="`${baseClass}__title`">Los 10 productos más comprados</h1>
            <UiIconButton
                icon="download"
        ...
        <div v-if="toggleOptions[0].selected" :class="`${baseClass}__chart-container`" ref="chartContainer">
            <UiPurchaseBarChart :top-products="topProducts" />
        </div>
        <div v-else-if="toggleOptions[1].selected" :class="`${baseClass}__chart-container`" ref="chartContainer">
            <UiPurchasePieChart :top-products="topProducts" />
        </div>
        <div v-else ref="chartContainer">
            <UiPurchaseProductsGrid :top-products="topProducts" />
        </div>
    </div>
</template>
...
import UiPurchaseBarChart from './ui-purchase-bar-chart.component.vue';
import UiPurchasePieChart from './ui-purchase-pie-chart.component.vue';
import UiPurchaseProductsGrid from './ui-purchase-products-grid.component.vue';
...
const selectCategory = async (value: string) => {
  ...
  topProducts.value =  await getTopPurchasedProducts([category.value]);
};
...
onMounted(async () => {
    ...
    topProducts.value = await getTopPurchasedProducts();
});
</script>
